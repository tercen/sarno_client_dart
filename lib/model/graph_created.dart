...
class GraphCreated {
  ...
  static GraphCreated? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'), 'Required key "GraphCreated[id]" is missing from JSON.');
        assert(json[r'id'] != null, 'Required key "GraphCreated[id]" has a null value in JSON.');
        assert(json.containsKey(r'revision'), 'Required key "GraphCreated[revision]" is missing from JSON.');
        assert(json[r'revision'] != null, 'Required key "GraphCreated[revision]" has a null value in JSON.');
        return true;
      }());
