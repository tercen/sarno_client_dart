...
class UserInfo {
  ...
  @override
  String toString() => 'UserInfo[username=$username, displayName=$displayName, email=$email, teams=$teams, roles=$roles, createdAt=$createdAt]';
  ...
  static UserInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'username'), 'Required key "UserInfo[username]" is missing from JSON.');
        assert(json[r'username'] != null, 'Required key "UserInfo[username]" has a null value in JSON.');
        assert(json.containsKey(r'roles'), 'Required key "UserInfo[roles]" is missing from JSON.');
        assert(json[r'roles'] != null, 'Required key "UserInfo[roles]" has a null value in JSON.');
        return true;
      }());
